...
class SQLiteLibraryRepo:
    ...
    def init(self) -> None:
        ...
        self._conn.commit()
        existing = {row[1] for row in self._conn.execute("PRAGMA table_info(library_items)").fetchall()}
        if "authors" not in existing:
            self._conn.execute("ALTER TABLE library_items ADD COLUMN authors TEXT DEFAULT ''")
        if "year" not in existing:
            self._conn.execute("ALTER TABLE library_items ADD COLUMN year INTEGER")
        self._conn.commit()
