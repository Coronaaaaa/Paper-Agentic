...
**主链**:
```text
PDF/DOCX 上传 → MinerU 解析 → VLM 图片描述 → 清洗 → 语义切块 → Embedding → Chroma + BM25
...
├── backend/                  # FastAPI 后端
│   ├── app/
│   │   ├── service_layer/    # API 路由 / DI 容器 / 配置 / SSE
│   │   ├── agent_layer/      # LLM 编排 / 检索 / 反思 / 工具调用
│   │   └── data_layer/       # PDF 解析 / 向量库 / BM25 / SQLite
│   ├── main.py               # 入口
│   └── pyproject.toml
├── tests/                    # 全项目测试（按 layer 组织）
...
| `EMBEDDING_API_KEY` / `EMBEDDING_BASE_URL` | Embedding 服务（硅基流动） |
...
测试按 layer 组织：`tests/{layer}/{unit,integration,e2e}/`，详见 [tests/README.md](tests/README.md)。
...
| POST | `/api/v1/import/start` | 上传导入（FormData） |
| GET | `/api/v1/import/stream/{id}` | 导入进度（SSE） |
| POST | `/api/v1/query` | 提问（SSE 流式） |
