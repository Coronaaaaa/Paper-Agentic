...
class TestDocU01:
    ...
    async def test_pdf_passes_format_check(self, tmp_dir):
        ...
        with patch(
            "app.data_layer.preprocessing.transfer.pipeline.PipelineOrchestrator"
        ) as MockPipeline:
            ...
            result = await service.ingest_document(pdf_path)

            mock_instance.run.assert_called_once()

    @pytest.mark.asyncio
    async def test_docx_rejected_by_format_check(self, tmp_dir):
        """DOCX 文件被格式检查拒绝"""
        service = _make_service(tmp_dir)
    ...
    @pytest.mark.asyncio
    async def test_non_pdf_rejected(self, tmp_dir):
        """非 PDF 格式全部被拒绝"""
        service = _make_service(tmp_dir)
        rejected = [".docx", ".doc", ".pptx", ".xlsx", ".ppt", ".xls"]

        for suffix in rejected:
            file_path = tmp_dir / f"test{suffix}"
            file_path.write_bytes(b"fake")
            result = await service.ingest_document(file_path)
            assert result.success is False, f"{suffix} 应被拒绝"
            assert "不支持的文件格式" in result.error, f"{suffix} 错误信息不正确"
