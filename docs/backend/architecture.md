...
- 通过 MinerU 精准解析 API 直接处理，无需探针和路由
...
#### 5.1.2 向量化与索引（`indexing/`）

数据层第二块负责把结构化文档变成可检索索引。对应代码子层 `indexing/`，包含 `embedding/` 和 `chroma_store/`。
...
数据层第三块负责文件管理、元数据持久化、配置和运行监控。对应代码子层 `storage/`，包含 `file_management/`、`sqlite_runtime/`、`config/`、`monitor/`。

主要负责：

- 原始文件副本管理与解析产物落盘
- SQLite 作为长期事实存储（文档元数据、导入记录、会话记录、摘要记录、锚点索引）
- 导入阶段中间产物和恢复点管理
- 运行态配置与监控数据

#### 5.1.4 混合检索（`retrieval/`）
