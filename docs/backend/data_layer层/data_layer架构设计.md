...
| 现有文件 | 新位置 | 处理方式 |
|---|---|---|
| `conversion/pdf_probe.py` | `PDF_preprocessor_data/probe/` | 搬迁，基本可用 |
| `conversion/pdf_router.py` | `PDF_preprocessor_data/transformation/` | 拆分：路由逻辑移入 transfer，转换逻辑保留 |
| `conversion/markitdown_adapter.py` | `PDF_preprocessor_data/transformation/` | 搬迁 |
| `conversion/docling_adapter.py` | 删除 | 死代码，Docling 不是当前默认链 |
| `normalization/vision_model.py` | `PDF_preprocessor_data/vlm_understanding/` | 重写为完整 pipeline |
| `chunking/semantic_chunker.py` | `PDF_preprocessor_data/chunking/` | 增强：加锚点、加嵌入相似度切分 |
| `indexing/embedding_client.py` | `data_persistence/embedding/` | 搬迁 |
| `storage/vector_index.py` | `data_persistence/chroma_store/` + `retrieval/dense/` | 拆分：写入 vs 查询 |
| `storage/keyword_index.py` | `data_persistence/chroma_store/` + `retrieval/sparse/` | 拆分：写入 vs 查询 |
| `storage/sqlite_runtime.py` | `data_persistence/chroma_store/` | 保留 library_items / import_tasks，conversation 迁移到 Agent 层 |
